...
echof info," mysqlshow indicates the presence of the database: $DBCHECK_RESULT"
...
if [[ "$C5_PRESEED" == yes ]]; then
  ...
  if [[ ! "$DBCHECK_RESULT" == "$DB_NAME" ]]; then
    ...
    # echof blank "please run this container with the environment variable C5_PRESEED set to no if you wish to start it without the database C5_PRESEED."
    # 1>&2 ; exit 1; }
  else
